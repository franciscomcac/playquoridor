...
ALTER TABLE public.open_rooms
  ADD COLUMN IF NOT EXISTS ranked boolean NOT NULL DEFAULT false;

ALTER TABLE public.matches
  ADD COLUMN IF NOT EXISTS ranked boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.apply_elo_1v1(
  _winner_player_id uuid,
  _winner_name text,
  _loser_player_id uuid,
  _loser_name text
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Ensure rows exist. auth_user_id is only tagged on the caller's own row
  -- so RLS on player_stats (auth_user_id IS NULL OR = auth.uid()) accepts writes.
  ...
  -- Ensure player rows exist so leaderboard joins resolve names.
...
GRANT EXECUTE ON FUNCTION public.apply_elo_1v1(uuid, text, uuid, text) TO authenticated;
