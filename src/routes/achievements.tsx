  ...
  head: () => ({
    meta: [
      { title: "Achievements · playquoridor.online" },
      {
        name: "description",
        content:
          "Unlock constellation sigils across 60+ Quoridor achievements — win streaks, rank tiers, wall mastery, puzzle streaks and hidden secrets.",
      },
      { property: "og:title", content: "Quoridor Achievements — playquoridor.online" },
      {
        property: "og:description",
        content:
          "60+ constellation sigils to unlock across wins, streaks, rank tiers and hidden secrets.",
      },
...
function AchievementsPage() {
  ...
  const grouped = useMemo(() => {
    ...
    return CATEGORY_ORDER.map((k) => ({ key: k, items: g.get(k) ?? [] })).filter(
      (c) => c.items.length > 0,
    );
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-10">
        <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-zinc-400">
              Unlock constellation sigils by winning, climbing rank, mastering walls and discovering
              hidden feats. Pin up to 3 to your profile.
            </p>
          </div>
          {signedIn && (
        ...
        <div className="mb-6 flex gap-2">
          ...
          {!signedIn && !meLoading && (
            <Link
              to="/auth"
              className="ml-auto rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium uppercase tracking-widest text-emerald-200 hover:bg-emerald-500/20"
            >
              Sign in to track
            </Link>
          )}
          ...
          <section className="mb-10">
            <h2 className="mb-4 text-[11px] font-semibold uppercase tracking-[0.3em] text-zinc-500">
              ...
              <span className="ml-2 text-zinc-700">
                {familyCards.filter((c) => c.currentLevel > 0).length}/{familyCards.length}
              </span>
            </h2>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {familyCards.map(({ fam, src, currentLevel, maxLevel, nextSlug }) => {
                ...
                const nextMeta = nextSlug
                  ? (catalog!.find((c) => c.slug === nextSlug) ?? null)
                  : null;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center gap-1">
                      {Array.from({ length: maxLevel }).map((_, i) => (
                        <span
                          ...
                          style={{
                            background:
                              i < currentLevel ? "rgb(52 211 153)" : "rgba(255,255,255,0.08)",
                          }}
                        />
                      ))}
                    </div>
                    ...
                    {progress && (
                      <div className="mt-2 w-full">
                        ...
                        <div className="mt-1 flex items-center justify-between font-mono text-[9px] uppercase tracking-widest text-zinc-500">
                          <span>
                            {Math.min(progress.current, progress.next).toLocaleString()} /{" "}
                    ...
                    {!progress && currentLevel >= maxLevel && (
                      <span className="mt-2 font-mono text-[9px] uppercase tracking-widest text-emerald-400/80">
                        Maxed
                      </span>
                    )}
                  </div>
