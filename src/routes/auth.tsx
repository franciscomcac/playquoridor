    ...
    <main className="relative min-h-screen bg-[#0d0d10] text-foreground flex items-center justify-center py-10 px-4 sm:px-8">
      ...
      <div className="relative z-10 w-full max-w-[440px]">
        ...
        <div className="rounded-2xl border border-[#232329] bg-[#111114] p-6 sm:p-8 shadow-2xl">
          <Link to="/" className="inline-flex items-center text-[11px] font-semibold uppercase tracking-[0.14em] text-[#5c5c66] hover:text-[#ececf1] transition-colors mb-6">
            ← Back to Home
          </Link>
          ...
          {hasAccount ? (
            <div className="rounded-[10px] border border-[#232329] bg-[#0d0d10] p-5">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#5c5c66]">Signed in as</p>
                  <p className="font-medium text-[#ececf1] text-[13px] truncate max-w-[200px]">{signedIn?.email ?? "your account"}</p>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center gap-4 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#5c5c66]">
                <span className="h-px flex-1 bg-[#232329]" />OR<span className="h-px flex-1 bg-[#232329]" />
              </div>

              <form onSubmit={onEmail} className="flex flex-col gap-4">
                <div className="space-y-[6px]">
                  <label className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#5c5c66]">Email</label>
                ...
                <div className="space-y-[6px]">
                  <div className="flex justify-between items-center">
                    <label className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#5c5c66]">Password</label>
                    {mode === "signin" && (
                ...
                {error && <div className="rounded-[10px] bg-[rgba(255,92,92,0.1)] border border-[#ff5c5c]/20 p-3 text-[12.5px] text-[#ff5c5c]">{error}</div>}
                {resetMsg && <div className="rounded-[10px] bg-[rgba(47,213,117,0.1)] border border-[#2fd575]/20 p-3 text-[12.5px] text-[#2fd575]">{resetMsg}</div>}
              ...
              <div className="mt-1 text-center">
                <button type="button" onClick={() => { setMode(mode === "signup" ? "signin" : "signup"); setError(null); setResetMsg(null); }}
