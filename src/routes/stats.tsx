...
import { Avatar, LobbyChrome, isPlacement, tierFromRating } from "@/components/LobbyChrome";
  ...
  head: () => ({
    meta: [
      { title: "Leaderboard · playquoridor.online" },
      {
        name: "description",
        content:
          "The global Quoridor leaderboard — top ranked players, tiers, win rates and rating changes.",
      },
      { property: "og:title", content: "Quoridor Leaderboard — playquoridor.online" },
      {
        property: "og:description",
        content: "The global Quoridor leaderboard — top ranked players, tiers, and rating changes.",
      },
    ...
    <LobbyChrome>
      <div className="mx-auto max-w-[1600px] px-8 pb-14 pt-[70px]">
        <div className="text-center">
          <div className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#f5a524]">
            {season.label} · {season.endsLabel}
          </div>
        ...
        <div className="mt-9 flex justify-center gap-2">
          {(["global", "friends"] as const).map((id) => (
            <button
              ...
              onClick={() => setTab(id)}
              className={
                "rounded-[9px] border px-5 py-[9px] text-[11.5px] font-bold uppercase tracking-[0.06em] " +
                (tab === id
        ...
        {rows === null ? (
          ...
          <p className="mt-16 text-center text-sm text-[#5c5c66]">
            {tab === "friends"
              ? "None of your friends have played ranked matches yet."
          ...
          <>
                ...
                {[podium[1]!, podium[0]!, podium[2]!].map((p, i) => {
                  ...
                  const cardCls =
                    rank === 1
                      ...
                      : rank === 2
                        ? "border-[rgba(200,205,215,0.28)] py-8"
                        : "border-[rgba(205,127,50,0.35)] py-8";
                  const medalBg =
                    rank === 1
                      ? "bg-[#f5c542] text-[#160e00] border-[#f5c542]"
                      : rank === 2
                        ? "bg-[#c8cdd7] text-[#111114] border-[#c8cdd7]"
                  ...
                  return (
                    <div
                      key={p.id}
                      className={
                        "relative rounded-[20px] border bg-[linear-gradient(180deg,#17171b,#111114)] px-6 pb-7 text-center " +
                        cardCls
                      }
                    >
                      <div
                        className={
                          "absolute left-1/2 top-[-14px] grid h-7 w-7 -translate-x-1/2 place-items-center rounded-full border font-[IBM_Plex_Mono,monospace] text-[12px] font-semibold " +
                          medalBg
                        }
                      >
                        {rank}
                      ...
                      <div
                        className="mt-[3px] text-[10px] font-bold uppercase tracking-[0.16em]"
                        style={{ color: t.color }}
                      >
                        {t.name}
                      </div>
                      <div className="mt-4 font-[IBM_Plex_Mono,monospace] text-[36px] font-semibold leading-none">
                        {p.rating}
                      </div>
                      <div className="mt-1 text-[11px] tracking-[0.1em] text-[#5c5c66]">
                        ELO RATING
                      </div>
                      <div className="mt-3 text-[12px] text-[#83838e]">
                        {p.matches} games · {wr}% wins
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="my-4 flex items-center justify-between">
              ...
              <span className="font-[IBM_Plex_Mono,monospace] text-[11px] tracking-[0.12em] text-[#5c5c66]">
                Δ 7D = RATING CHANGE, LAST 7 DAYS
              </span>
            </div>
...
function LbRow({ rank, row, me }: { rank: number; row: FullLeaderRow; me?: boolean }) {
  ...
  return (
    <div
      className={
        "grid items-center gap-3 border-t border-[#1a1a1f] px-6 py-[15px] md:grid-cols-[40px_32px_minmax(0,1fr)_90px_90px_80px_90px_70px] " +
    ...
    >
      ...
      <span className="hidden text-right font-[IBM_Plex_Mono,monospace] text-[12.5px] text-[#83838e] md:block">
        {row.matches}
      </span>
      <span className="hidden text-right font-[IBM_Plex_Mono,monospace] text-[12.5px] text-[#83838e] md:block">
        {wr}%
      ...
      <span className="text-right font-[IBM_Plex_Mono,monospace] text-[12.5px] font-semibold text-[#ececf1]">
        {row.rating}
      </span>
      <span
        className={
          "text-right font-[IBM_Plex_Mono,monospace] text-[12.5px] " +
          (up ? "text-[#2fd575]" : "text-[#ff7a7a]")
        }
      >
        {up ? "+" : ""}
        {row.delta7d}
