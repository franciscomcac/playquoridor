...
export function QueuePuzzle() {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      if (!next) return;
      setState(next); play("pop");
      if (bot.kind === "pawn" && reachedGoal(bot.to, oppGoal)) {
        setStatus("failed"); window.setTimeout(() => play("denied"), 120);
      }
    }, 500);
  ...
  const onMove = useCallback((m: Move) => {
    ...
    if (m.kind === "pawn" && reachedGoal(m.to, yourGoal)) {
      setStatus("solved"); window.setTimeout(() => play("roundWin"), 120);
    }
  }, [state, status, yourGoal]);

  const reset = () => {
    setState(buildPuzzleGameState(puzzle));
    setStatus("playing"); busy.current = false;
  };
  const nextPuzzle = () => {
