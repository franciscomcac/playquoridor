...
const TIER_STYLES: Record<TierKey, { name: string; c1: string; c2: string; glow: string; size: number }> = {
...
const TIER_ORDER: TierKey[] = ["bronze","silver","gold","platinum","diamond","master","top10"];
...
export function RankUpOverlay({
  oldRating, newRating, onDone,
}: { oldRating: number; newRating: number; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    setPhase("idle"); setBadgeStage("old"); setEloNow(oldRating);
    ...
    const tick = () => {
      ...
      if (elapsed >= tl.showAt && !flags.show) { flags.show = true; setPhase("show"); }
      if (elapsed >= tl.breakAt && !flags.brk) { flags.brk = true; if (tierUp) setBadgeStage("breaking"); }
      if (elapsed >= tl.popAt && !flags.pop) { flags.pop = true; setBadgeStage("new"); }
      ...
      if (elapsed >= exitAt && !flags.exit) { flags.exit = true; setPhase("exit"); }
  ...
  const framePhaseClass = (phase === "show") ? "ru-show" : (phase === "exit" || phase === "gone" ? "ru-exit" : "");
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center overflow-hidden"
      ...
      <div className="pointer-events-none absolute inset-0"
        style={{ background: `radial-gradient(ellipse 900px 560px at 50% 46%, rgba(${toStyle.glow},.14), transparent 70%)` }} />
      <RankUpStyles />
      <div className={"ru-frame relative z-[1] flex flex-col items-center gap-6 " + framePhaseClass}>
        <div className="text-[12px] font-bold uppercase tracking-[0.22em] text-zinc-500">Rank up</div>

        <div className="relative grid h-[220px] w-[220px] place-items-center">
          {rayEligible && badgeStage === "new" && (
            <div className="ru-rays absolute inset-[-70px] rounded-full"
              style={{ background:
          ...
          {showCrown && (
            <div className="ru-crown absolute -top-4 left-1/2 z-[4] -translate-x-1/2 whitespace-nowrap rounded-full px-3 py-[5px] text-[10.5px] font-extrabold uppercase tracking-[0.09em] text-[#1a1200]"
              style={{
          ...
          {particles.map((pt, i) => (
            <span key={i}
              className={"absolute left-1/2 top-1/2 h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full " + (badgeStage === "new" ? "ru-burst" : "opacity-0")}
              style={{
                background: pt.color,
...
function Badge({ style, className = "" }: { style: (typeof TIER_STYLES)[TierKey]; className?: string }) {
  return (
    <div className={"grid place-items-center rounded-full " + className}
      ...
      <div className="h-[38%] w-[38%] rotate-45"
        style={{
