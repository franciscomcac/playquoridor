...
export function CreateRoom({ mode, walls, rounds, setMode, setWalls, setRounds, onBack, onStart }: {
...
export function JoinRoom({ onBack, onJoin }: { onBack: () => void; onJoin: (code: string) => void }) {
...
export function SpectateRoom({ onBack, onJoin }: { onBack: () => void; onJoin: (code: string) => void }) {
  const [code, setCode] = useState("");
  const clean = code.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 5);
  return (
    <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl sm:p-8">
