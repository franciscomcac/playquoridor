    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-sm aborted-fade">
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <div className="aborted-text">
          <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-rose-400">
            Match ended
          </p>
    ...
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded-lg bg-background/60 backdrop-blur-[4px] overflow-hidden">
      ...
      <div
        ...
        style={{
          ...
          WebkitMaskImage:
            "radial-gradient(circle at center, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0) 55%)",
        }}
      />
          ...
          return (
            <span
              ...
              style={
                filled
                  ? undefined
                  : { animation: `wo-pulse 1.6s ease-in-out ${i * 0.18}s infinite` }
              }
            />
          );
        })}
      ...
      {isHost && count >= 2 && count < expected && (
        <button
          onClick={onStart}
          className="mt-4 rounded-lg bg-emerald-500 px-4 py-2 text-[11px] font-semibold uppercase tracking-widest text-emerald-950 shadow-[0_15px_40px_-15px_rgba(16,185,129,0.7)] transition-transform hover:-translate-y-0.5"
        >
          Start with {count}
        </button>
      )}
  ...
  return (
    <div className="mt-3 w-full max-w-md rounded-lg border border-border bg-secondary/30 px-4 py-3 text-center">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Save your games, chat in-match, and get a rating.
      </p>
      <Link
        to="/auth"
