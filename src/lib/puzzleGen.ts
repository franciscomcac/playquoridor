...
export function generatePuzzle(seed: number, difficulty: 1 | 2 | 3 = 2): GeneratedPuzzle {
  ...
  const oppCol =
    oppColBase === you[1] ? (oppColBase + (rng() < 0.5 ? -1 : 1) + BOARD) % BOARD : oppColBase;
...
export function buildPuzzleGameState(
  p:
    | GeneratedPuzzle
    | {
        mode: number;
  ...
  const wallsLeft = Array.from({ length: mode }, (_, i) =>
    i === 0 ? playerWalls : i === 1 ? oppWalls : 0,
  );
  return {
    mode,
    pawns,
