...
export const FAMILIES: Family[] = [
  {
    ...
    name: "Wins",
    category: "wins",
    slugs: ["win_10", "win_50", "win_100", "win_500", "win_1000"],
  },
  {
    id: "streaks",
    name: "Win Streaks",
    category: "streaks",
    slugs: ["streak_3", "streak_5", "streak_10", "streak_20"],
  },
  {
    id: "walls",
    name: "Walls Placed",
    category: "walls",
    slugs: ["walls_100", "walls_500", "walls_2000"],
  },
  {
    id: "matches",
    name: "Matches",
    category: "milestones",
    slugs: ["matches_10", "matches_100", "matches_500", "matches_2000"],
  },
  { id: "rounds", name: "Rounds", category: "milestones", slugs: ["rounds_500", "rounds_2000"] },
  {
    ...
    name: "Rank",
    category: "rank",
    slugs: ["rank_1200", "rank_1400", "rank_1600", "rank_1800", "rank_2000", "rank_2200"],
  },
  ...
  { id: "mode_4p", name: "Chaos Mode", category: "modes", slugs: ["mode_4p_25", "mode_4p_100"] },
  {
    ...
    name: "Puzzles",
    category: "puzzles",
    slugs: ["puzzle_7", "puzzle_30", "puzzle_100"],
  },
  {
    id: "friends",
    name: "Friends",
    category: "social",
    slugs: ["friend_1", "friend_5", "friend_20"],
  },
  ...
  { id: "banner", name: "Ambassador", category: "identity", slugs: ["flag_bearer", "ambassador"] },
  {
    ...
    name: "Giant Slayer",
    category: "skill",
    slugs: ["giant_slayer", "giant_slayer_10"],
  },
];
...
const SLUG_INDEX: Map<string, { familyId: string; level: number; maxLevel: number }> = (() => {
  ...
  for (const fam of FAMILIES) {
    fam.slugs.forEach((s, i) =>
      m.set(s, { familyId: fam.id, level: i + 1, maxLevel: fam.slugs.length }),
    );
  }
  return m;
})();
...
export function familyOf(
  slug: string,
): { familyId: string; level: number; maxLevel: number } | null {
...
export function familyView(
  fam: Family,
  catalog: Map<string, SlugMeta>,
  unlocked: Set<string>,
): FamilyView {
  let currentLevel = 0;
  let currentSlug: string | null = null;
  for (let i = fam.slugs.length - 1; i >= 0; i--) {
    if (unlocked.has(fam.slugs[i])) {
      currentLevel = i + 1;
      currentSlug = fam.slugs[i];
      break;
    }
  }
...
export function partitionCatalog(
  ...
  const families = FAMILIES.filter((fam) => fam.slugs.some((s) => byId.has(s))).map((fam) =>
    familyView(fam, byId, unlocked),
  );
